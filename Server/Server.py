...
class Server():
    ...
    def receive_hb(self):
        ...
        leader_heartbeat = self.readClient(4444,False,heartbeat_leader=False,heatbeat_server=True)    #fix this port to own heartbeat port
        print("📩 LEADER_HEARTBEAT_RECEIVED: ",leader_heartbeat)
        if leader_heartbeat:
            if leader_heartbeat[1] == b'heartbeat':
                time.sleep(1)
                thread = threading.Thread(target=self.writeToClient, args=('HB_received', self.leader, local_server_port,))    #fix localserverport to leader heartbeat
                thread.start()
                thread.join()
        else:
            if self.is_leader:
    ...
    def send_heartbeat(self):
        for server in self.group_view:
            ...
            if server_ip != self.leader:
                ...
                async_result = pool.apply_async(self.readClient, (local_server_port,False,True,False))  # tuple of args for foo
                ...
                if listen_heartbeat:
                    if listen_heartbeat[1] == b'HB_received':
                        ...
                        self.hbList[listen_heartbeat[0][0]] = 0      #later make this ip
                else:
                    if self.hbList[server_ip] > 3:   #later make this ip and change to 3 tries i.e 2
                        print("\nServer {} {} is dead:".format(server_ip,server_id))
                        self.hbList[server_ip] = 0   #later make this ip
                    ...
                    self.hbList[server_ip] = self.hbList[server_ip] + 1     #later make this ip

    def hbMechanism(self):
        while True:   #shud this while loop be inside heartbeating
            if self.is_leader:
                is_leader = self.send_heartbeat()    #should this start new thread
                if is_leader:
    ...
    def acptJoin(self, server):
        while True:
            ...
            try:
                ...
                self.hbList[newServerIP.decode()] = 0     ##made this change
                message = pickle.dumps(self.group_view)
                print(message)
                self.send_to_all_servers(server, message, 5044)
                LeaderSS.close()
    ...
    def collectClients(self,chatrooms,chatroom_inport,chatroom_outport):
        for chatroom in self.chatrooms_handled:
            ...
            for client in chatroom['clients_handled']:
                Chat_ = threading.Thread(target=self.writeToChatroom, args=(chatrooms,chatroom_inport,chatroom_outport,))
                Chat_.start()
        for chatroom in self.chatrooms_handled:
            if len(chatroom['clients_handled']) == 0:
                continue
    ...
    def writeToChatroom(self,chatrooms,chatroom_inport,chatroom_outport):
        while True:
            bytesAddressPair = self.readClient(chatroom_inport,True)  # localPort_in for each chatroom
...
def heartbeats():
    while True:
        if sObj.is_leader == True:
            pool1 = ThreadPool(processes=1)
            async_result = pool1.apply_async(sObj.hbMechanism, ())  
            listen_heartbeat = async_result.get()
            if listen_heartbeat:
                return
        else:
            pool2 = ThreadPool(processes=1)
            async_result = pool2.apply_async(sObj.hbMechanism, ())  
            listen_heartbeat = async_result.get()
            if listen_heartbeat:
                return
...
if __name__ == "__main__":
    sObj = Server()
    sObj.joinNwk(sObj)
    Chat_ = threading.Thread(target=sObj.collectChatroom, args=())
    Chat_.start()
    c_election = threading.Thread(target = sObj.election, args = (sObj,))
    c_election.start()
    while True:
        if sObj.is_leader == True:
            _join = threading.Thread(target = sObj.acptJoin, args = (sObj,))
            _join.start()
            _login = threading.Thread(target = sObj.acptLogin, args = (sObj,))
            _login.start()
            c_heart = threading.Thread(target=heartbeats, args=())
            c_heart.start()
            _login.join()
            _join.join()
        else:
            _groupviewUpdate = threading.Thread(target = sObj.updateGrpView, args = (sObj,))
            _groupviewUpdate.start()
            _clientUpdate = threading.Thread(target = sObj.updateClientList, args = (sObj,))
            _clientUpdate.start()
            c_election = threading.Thread(target = sObj.election, args = (sObj,))
            c_election.start()
            c_heart = threading.Thread(target= heartbeats, args=())
            c_heart.start()
            _groupviewUpdate.join()
            _clientUpdate.join()
